...
export async function getNewRedisClient(host?: string): Promise<any> {
	...
	const _redis_client: any = redis.createClient({
		host,
	...
	_redis_client.on('ready', function (data: any) {
		console.log(`Redis ready`, _redis_client.connection_options);
	})
	...
	for (const m of methods) {
		client[m] = promisify(_redis_client[m]).bind(_redis_client);
	}
